...
async function handleDetection(request: Request) {
  try {
    ...
    // Traitement par lots (Batches) avec temporisation pour respecter le Rate Limit par minute
    for (let i = 0; i < clubs.length; i += batchSize) {
      const currentBatch = clubs.slice(i, i + batchSize);

      await Promise.all(currentBatch.map(async (club) => {
      ...
      // Si nous avons encore des lots à traiter, nous faisons une pause de 15 secondes pour respecter le quota par minute

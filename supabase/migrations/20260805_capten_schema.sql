-- CAPTEN — Schema complet per specifications
...
CREATE POLICY IF NOT EXISTS "Clubs are viewable by everyone" ON clubs FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Organizers can create clubs" ON clubs FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'organizer')
);
CREATE POLICY IF NOT EXISTS "Owners can update their clubs" ON clubs FOR UPDATE USING (auth.uid() = owner_id);
...
CREATE POLICY IF NOT EXISTS "Members viewable by everyone" ON club_members FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Users can join clubs" ON club_members FOR INSERT WITH CHECK (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Club admins can update members" ON club_members FOR UPDATE USING (
...
CREATE POLICY IF NOT EXISTS "Published events are public" ON events FOR SELECT USING (status = 'published' OR status = 'completed');
CREATE POLICY IF NOT EXISTS "Club admins can see all events" ON events FOR SELECT USING (
  EXISTS (SELECT 1 FROM club_members cm WHERE cm.club_id = events.club_id AND cm.member_id = auth.uid())
);
CREATE POLICY IF NOT EXISTS "Club admins can manage events" ON events FOR ALL USING (
...
CREATE POLICY IF NOT EXISTS "Registrations viewable by club admins and self" ON event_registrations FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Users can register themselves" ON event_registrations FOR INSERT WITH CHECK (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Users can update own registration" ON event_registrations FOR UPDATE USING (auth.uid() = member_id);
...
CREATE POLICY IF NOT EXISTS "Checkins viewable by self and admins" ON checkins FOR SELECT USING (
  auth.uid() = member_id OR
...
CREATE POLICY IF NOT EXISTS "Users can check themselves in" ON checkins FOR INSERT WITH CHECK (auth.uid() = member_id);
...
CREATE POLICY IF NOT EXISTS "Users manage own ICE info" ON ice_contacts FOR ALL USING (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Admins can view ICE info" ON ice_contacts FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "View own waivers" ON waivers FOR SELECT USING (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Insert own waivers" ON waivers FOR INSERT WITH CHECK (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Admins view club waivers" ON waivers FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Spots viewable by public" ON spots FOR SELECT USING (is_active = true);
CREATE POLICY IF NOT EXISTS "Club admins manage spots" ON spots FOR ALL USING (
...
CREATE POLICY IF NOT EXISTS "Club admins view transactions" ON spot_transactions FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Club admins insert transactions" ON spot_transactions FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM club_members cm WHERE cm.club_id = spot_transactions.club_id AND cm.member_id = auth.uid() AND cm.role IN ('admin', 'co_organizer'))
);
...
CREATE POLICY IF NOT EXISTS "Badges are public" ON badges FOR SELECT USING (is_active = true);
...
CREATE POLICY IF NOT EXISTS "Member badges are public" ON member_badges FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Insert member badges" ON member_badges FOR INSERT WITH CHECK (true);

-- member_tokens (micro-page access)
...
CREATE POLICY IF NOT EXISTS "View own tokens" ON member_tokens FOR SELECT USING (auth.uid() = member_id);
CREATE POLICY IF NOT EXISTS "Public can read tokens for micro-page" ON member_tokens FOR SELECT USING (is_active = true);
...
-- Auto-create member token on profile insert
CREATE OR REPLACE FUNCTION create_member_token()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO member_tokens (member_id)
  VALUES (NEW.id)
  ON CONFLICT DO NOTHING;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_create_member_token
AFTER INSERT ON profiles
FOR EACH ROW EXECUTE FUNCTION create_member_token();
